...
assert(sources["TRH_AutoTrade.mq5"].includes("v3.38"), "EA version 3.38");
assert(sources["TRH_Trading_Room_Hunter.mq5"].includes("TRH_IND_BUILD 233"), "Indicator build 233");
assert(sources["TRH_Trading_Room_Hunter.mq5"].includes("TRH_MIN_ENGINE 231"), "Indicator requires Engine 231");
assert(sources["TRH_Trading_Room_Hunter.mq5"].includes("InpOnlyLast        = true"), "OnlyLast matches Pine default");
assert(sources["TRH_Engine.mqh"].includes("cfg.minFvgAtr       = 0.28"), "Engine minFvg 0.28 rejects micro gaps");
assert(sources["TRH_Engine.mqh"].includes("cfg.fvgMinRiskAtr   = 1.15"), "Engine Mode B min risk 1.15 ATR");
assert(sources["TRH_Engine.mqh"].includes("cfg.fvgSlExtraAtr   = 0.35"), "Engine Mode B SL extra 0.35");
assert(sources["TRH_Engine.mqh"].includes("(gapTop + gapBot) * 0.5"), "Engine Mode B entry = FVG mid");
