//+------------------------------------------------------------------+
//| ICT_AutoTrade.mq5                                                |
//| Standalone ICT EA (NOT TRH)                                      |
//| Sweep -> MSS -> FVG -> opposing liquidity                        |
//+------------------------------------------------------------------+
#property copyright "ICT Liquidity Expansion"
#property link      "https://github.com/radiarkazemi/forge-charts"
#property version   "1.00"
#property description "ICT EA: raid -> MSS -> FVG, closed-bar only"
#property strict
...
bool SpreadOk()
{
   ...
   {
      PrintFormat("ICT: skip - spread %d > max %d", (int)spread, InpMaxSpreadPoints);
      return false;
   }
   return true;
}
...
bool PlaceSetupTrade(const IctSetup &s, const double atrNow)
{
   ...
   string comment = StringFormat("ICT %s RAID", s.dir == 1 ? "LONG" : "SHORT");
   bool ok = false;
   string mode = "";
   ...
   {
      g_dayTrades++;
      PrintFormat("ICT %s %s lots=%s E=%s SL=%s TP=%s",
   ...
   {
      PrintFormat("ICT ORDER FAIL %s ret=%d %s",
...
int OnInit()
{
   ...
   {
      Alert("ICT EA: Engine missing. Put ICT_Engine.mqh in SAME folder and recompile.");
      return INIT_FAILED;
   }
   g_trade.SetExpertMagicNumber(InpMagic);
   g_trade.SetDeviationInPoints(InpMaxSlippagePts);
   g_trade.SetTypeFillingBySymbol(_Symbol);
   ResetDayIfNeeded();
   PrintFormat("ICT AutoTrade v1.00 | AutoTrade=%s | %s %s | risk=%.2f%%",
      InpAutoTrade ? "ON" : "OFF", _Symbol, EnumToString(_Period), InpRiskPercent);
   Comment("ICT EA v1.00\nRaid -> MSS -> FVG\nMT5 Watch bridge");
   WatchEmit("ICT", "info", "", "INIT", 0, 0, 0, "", "ICT EA attached + watch bridge on", 0);
...
void OnTick()
{
   ...
   ExpireStalePendings();
   ManageBreakEven();
   WatchHeartbeat("ICT", "alive", "OnTick");
   ...
   if(n <= 0)
   {
      Comment(StringFormat("ICT EA v1.00 %s - scanning...\nday %d",
         InpAutoTrade ? "ON" : "OFF", g_dayTrades));
      WatchStatus("ICT", 0, 0, 0, 0, "", "SCANNING", 0);
      return;
   }

   IctSetup last = setups[n - 1];
   ...
   WatchStatus("ICT", last.dir, last.entry, last.sl, last.tp, "RAID-MSS-FVG",
               StringFormat("LIVE age=%d", ageBars), last.barTime);
   ...
   PrintFormat("ICT SETUP %s E=%s SL=%s TP=%s sweep=%s",
      last.dir == 1 ? "LONG" : "SHORT",
      DoubleToString(last.entry, _Digits),
      DoubleToString(last.sl, _Digits),
      DoubleToString(last.tp, _Digits),
   ...
   WatchSetup("ICT", last.dir, last.entry, last.sl, last.tp, "RAID-MSS-FVG", last.barTime,
              StringFormat("sweep=%s", DoubleToString(last.sweepPrice, _Digits)));

   if(InpAlertOnSetup)
   {
      Alert(StringFormat("ICT %s SETUP\nENTRY %s\nSL %s\nTP %s",
         last.dir == 1 ? "LONG" : "SHORT",
         DoubleToString(last.entry, _Digits),
         DoubleToString(last.sl, _Digits),
         DoubleToString(last.tp, _Digits)));
