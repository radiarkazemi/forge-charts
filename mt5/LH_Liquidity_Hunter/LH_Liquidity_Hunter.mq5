...
//| Liquidity Hunter indicator (NOT TRH)                               |
//| Raid -> CISD -> MSS/BOS -> FVG -> opposing liquidity TP             |
//+------------------------------------------------------------------+
#property copyright "LH Liquidity Hunter"
#property link      "https://github.com/radiarkazemi/forge-charts"
#property version   "1.00"
#property description "LH: liquidity raid -> CISD -> MSS/BOS -> FVG -> opposing liq"
...
input int    InpMaxCisdBars      = 8;      // Max Bars After Raid For CISD
input bool   InpRequireCisd      = true;   // Require CISD Before Structure
input bool   InpAllowBos         = true;   // Allow Internal BOS (else MSS only)
input int    InpMaxStructBars    = 12;     // Max Bars After CISD For MSS/BOS
input int    InpMaxFvgBars       = 10;     // Max Bars After Structure For FVG
...
int OnInit()
{
   ...
   {
      Alert("LH: Engine missing/outdated. Put ICT_Engine.mqh next to this .mq5 and recompile.");
      return INIT_FAILED;
   }
   IndicatorSetString(INDICATOR_SHORTNAME, "LH Liquidity Hunter");
...
string StatusText(const LhSetup &s, const double &high[], const double &low[],
...
void DrawInfoPanel(const LhSetup &s, const string stTxt, const int stCode, const double bid)
{
   ...
   int y = 18;
   SetPanelLabel(OBJ_PREFIX + "P0", 18, y, "LH | Liquidity Hunter", clrWhite, 10); y += 18;
   SetPanelLabel(OBJ_PREFIX + "P1", 18, y,
      (s.dir == 1 ? "LONG" : "SHORT") + " | RAID->CISD->" + LhStructLabel(s.structType) + "->FVG | " + stTxt, stCol, 9); y += 16;
...
void DrawOneSetup(const LhSetup &s, const datetime &time[], const double &high[],
                  const double &low[], const double &close[], const int rates)
{
   ...
   SetLabel(OBJ_PREFIX + "TAG_" + tag, t1,
      ...
      "LH " + (s.dir == 1 ? "LONG" : "SHORT") + " | " + LhStructLabel(s.structType),
...
void NotifyNewSetup(const LhSetup &s)
{
   string msg = StringFormat(
      "LH %s RAID->CISD->%s->FVG\nENTRY %s\nSL %s\nTP %s\nSweep %s",
      s.dir == 1 ? "LONG" : "SHORT",
      LhStructLabel(s.structType),
                ...
                const int &spread[])
{
   ...
   if(newClosed || prev_calculated == 0)
   {
      ...
      if(g_nSetups > 0)
      {
         LhSetup newest = g_setups[g_nSetups - 1];
         if(!holdActive)
         {
            g_holdSetup = newest;
            g_holdValid = true;
